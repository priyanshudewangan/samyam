...
type YatraSlug = keyof typeof yatraDetailsDb;

function ExploreYatrasPage() {
  const search = Route.useSearch();
  const initialSlug = (search.yatra as YatraSlug) || "ayodhya-kashi";
  
  // Local state to manage active yatra selection
  const [selectedYatra, setSelectedYatra] = useState<YatraSlug>(initialSlug);
  ...
  useEffect(() => {
    if (search.yatra && yatraDetailsDb[search.yatra as YatraSlug]) {
      setSelectedYatra(search.yatra as YatraSlug);
    }
  }, [search.yatra]);

  const activeYatra = yatraDetailsDb[selectedYatra] || yatraDetailsDb["ayodhya-kashi"];
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-background text-primary">
      ...
      <section className="relative min-h-[50vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center pt-20">
          <span className="inline-block px-3 py-1 rounded-full bg-accent/25 border border-accent text-accent text-[10px] md:text-xs font-semibold uppercase tracking-[0.2em] mb-4">
      ...
      <div className="bg-white border-b border-[#f5e3e6]/80 sticky top-16 z-30 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-wrap items-center justify-center gap-2 md:gap-4">
          {[
            { slug: "ayodhya-kashi", label: "Ayodhya - Kashi", icon: "🕉️" },
            { slug: "rishikesh-badrinath", label: "Rishikesh Badrinath", icon: "🏔️" },
            { slug: "himachal", label: "Shakti Peethas", icon: "🌺" },
          ].map((item) => (
            <button
              key={item.slug}
              onClick={() => {
                setSelectedYatra(item.slug as YatraSlug);
              ...
              <div className="space-y-12">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
                    ...
                    <div className="p-6 rounded-3xl bg-[#fffbfb] border border-[#f5e3e6] space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        <div className="flex-1 p-3.5 rounded-2xl bg-white border border-[#f5e3e6] flex flex-col justify-center items-center shadow-soft">
                          ...
                          <span className="text-sm md:text-base font-semibold text-[#e35928] mt-1">{activeYatra.triplePrice}</span>
                        </div>
                        <div className="flex-1 p-3.5 rounded-2xl bg-white border border-[#f5e3e6] flex flex-col justify-center items-center shadow-soft">
                          <span className="text-[10px] text-muted-foreground uppercase font-bold">Double Occupancy</span>
                          <span className="text-sm md:text-base font-semibold text-[#822d56] mt-1">{activeYatra.doublePrice}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="lg:col-span-5 p-8 rounded-3xl bg-[#4e2055] text-white space-y-6 shadow-glow relative overflow-hidden text-left">
                    ...
                    <ul className="space-y-4 text-xs md:text-sm text-white/85">
                      {activeYatra.inclusions.map((item, idx) => (
                ...
                <div className="border-t border-[#f5e3e6] pt-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
                    {activeYatra.darshans.map((category, cIdx) => (
                      <div
                        key={cIdx}
            ...
            {activeSubTab === "itinerary" && (
              <div className="max-w-4xl mx-auto text-left">
                ...
                <div className="relative border-l border-[#f5e3e6] ml-4 md:ml-8 pl-8 md:pl-10 space-y-8 py-4">
                  {activeYatra.itinerary.map((dayItem, idx) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto text-left">
                ...
                <div className="p-8 rounded-3xl bg-gradient-to-br from-[#fffbfb] to-[#fbf2fa] border border-[#f5e3e6] shadow-soft space-y-6">
                  ...
                  <ul className="space-y-4 text-xs md:text-sm text-muted-foreground">
                    ...
                    <li className="flex gap-3 items-start leading-relaxed">
                      <span className="text-accent font-bold mt-0.5">•</span>
                      <span>Maintaing a clean, devotional mindset leading up to departure.</span>
                    </li>
                  </ul>
                </div>

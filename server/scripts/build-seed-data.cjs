...
yatraContent = yatraContent.replace(
  /img:\s*rishikeshBadrinathImg/g,
  'img: "/images/rishikesh_badrinath.png"',
);
yatraContent = yatraContent.replace(
  /img:\s*shaktiPeethasImg/g,
  'img: "/images/shakti_peethas.png"',
);
...
yatraContent = yatraContent.replace(
  /export\s+const\s+yatraDetailsDb[\s\S]*?=\s*/,
  "module.exports = ",
);

// Write temporary js file to evaluate it
const tempYatraPath = path.join(__dirname, "temp-yatra.cjs");
...
// Convert to module export
teerthaContent = teerthaContent.replace(
  /export\s+const\s+teerthaDetailsDb[\s\S]*?=\s*/,
  "module.exports = ",
);

const tempTeerthaPath = path.join(__dirname, "temp-teertha.cjs");
...
const tempJwPath = path.join(__dirname, "temp-jw.cjs");
